...
//
//        try {
//            String httpUrl = "http://" + inst.getHostName() + ":" + inst.getPort() + "/albums";
//            logger.info("Trying to get albums again with custom RestTemplate and custom built HTTP url: {}", httpUrl);
//            results = noLoadBalancedRestTemplate.exchange(httpUrl, GET, null, albumListType).getBody();
//        }
//        catch (Exception e) {
//            logger.warn("Was not able to get albums via custom HTTP URL");
//            logger.error("Ex Type: {}. Message was: {}. Causing message: {}", e.getClass().getName(), e.getMessage(), e.getCause() != null ? e.getCause().getMessage() : "<UNKNOWN>");
//        }
//
//        return results;
